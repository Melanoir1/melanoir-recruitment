...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabase.from('mnr_waitlist').upsert(
    {
      ...
      shop_name: type === 'pro' && body.shop_name ? String(body.shop_name).slice(0, 80) : null,
  ...
  if (!existing) {
    await sendSms(phone, '[멜라누아] 출시 알림 신청이 완료되었습니다. 정식 출시 소식을 가장 먼저 보내드릴게요.')
  }
